...
class WikipediaSpeedrun():
    ...
    def __init__(self,thread_limit:int,coordinates:list) -> None:
        self.stop_threads=False
        self.answer=[]
        self.seen=set()
        self.thread_limit = thread_limit
        temp_input = self.cheack_cords(coordinates)
    ...
    def cheack_cords(self,coordinates): #<==collects input and converts it to wikipedia links
        starting_url = wiki.opensearch(coordinates[0], results=1) #input("Start:")
        destination_url = wiki.opensearch(coordinates[1], results=1) #input("Destination:")
        ...
        while True:
            ...
            for id,link_container in enumerate(all_links_container):
                if self.stop_threads:
                    break
                thread_list.append(Thread(target=self.thread_process,args=(link_container,temp)))
                if id%self.thread_limit==0 or id==len(all_links_container)-1:  
                    for thread in thread_list:
                        thread.start()
                    for thread in thread_list:
                        thread.join()
    ...
    def print_result(self):
        ...
        return [time.perf_counter()-self.start_time,self.answer[0]]
...
if __name__=="__main__":
    speedrun = WikipediaSpeedrun(10,["Fall of Constantinople","Persepolis"])
    print(speedrun.main())
